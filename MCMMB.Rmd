...
#test_sdms$`Pachycephala soror`[[2]][[1]]
  attributes(gbif_sdms[["var_imp"]][[names(gbif_birds[i])]])[["dimnames"]][[1]]
  attributes(gbif_sdms[["var_imp"]][["Pachycephala schlegelii"]])[["dimnames"]][[1]] = c("a,b,c,d")
#names(test_sdms$`Pachycephala soror`[[2]])
```
```{r ensemble, results = 'hide'}

# plotting and evaluations for the ensemble models
for (i in 1:length(gbif_birds)) {
  plot(BIOMOD_EnsembleForecasting(EM.output = gbif_sdms$ensemble[[i]],
                                  projection.output = gbif_sdms$sdm_proj[[i]],
                                  selected.models = 'all'))
  get_evaluations(gbif_sdms$ensemble[[i]])
  }
```
